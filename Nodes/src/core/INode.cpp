...
// #include <Nodes/types/DummyObject.h>
...
// #include <Nodes/extra/TempNode.h>
// #include <Nodes/utils/Error.h>
#ifdef ZENO_BENCHMARKING
#include <Nodes/utils/Timer.h>
#endif
...
#include <filesystem>
#include <fstream>
// #include <Nodes/extra/GlobalComm.h>
// #include <Nodes/types/PrimitiveObject.h>

namespace NodeEditor {
int INode::IdCount = 0;
NODE_API INode::INode() = default;
NODE_API INode::~INode() = default;

// NODE_API Graph *INode::getThisGraph() const {
//     return graph;
// }

// NODE_API Session *INode::getThisSession() const {
//     return graph->session;
// }

// NODE_API GlobalState *INode::getGlobalState() const {
//     return graph->session->globalState.get();
// }

// NODE_API void INode::doComplete() {
//     set_output("DST", std::make_shared<DummyObject>());
//     complete();
// }

NODE_API void INode::complete() {}

/*NODE_API bool INode::checkApplyCondition() {
...
// NODE_API bool NodeEditor::INode::getTmpCache()
// {
...
// NODE_API void NodeEditor::INode::writeTmpCaches()
// {
//     GlobalComm::ViewObjects objs;
...
//
//     }
//     int frameid = NodeEditor::getSession().globalState->frameid;
//     std::string fileName = myname + ".zenocache";
//     GlobalComm::toDisk(NodeEditor::getSession().globalComm->objTmpCachePath, frameid, objs, false, false, fileName);
// }

// NODE_API void INode::preApply() {
...
//         std::string fileName = myname + ".zenocache";
//         int frameid = NodeEditor::getSession().globalState->frameid;
//         const auto& path = std::filesystem::u8path(NodeEditor::getSession().globalComm->objTmpCachePath + "/" + std::to_string(1000000 + frameid).substr(1) + "/" + fileName);
//         if (std::filesystem::exists(path))
//         {
//             std::filesystem::remove(path);
//             NodeEditor::log_info("remove cache file: {}", path.string());
//         }
//     }
//     for (auto const &[ds, bound]: inputBounds) {
//         requireInput(ds);
...
NODE_API bool INode::requireInput(std::string const &ds) {
...
NODE_API void INode::doOnlyApply() {
    apply();
}

NODE_API void INode::doApply() {
...
/*NODE_API bool INode::has_option(std::string const &id) const {
    return options.find(id) != options.end();
}*/

NODE_API bool INode::has_input(std::string const &id) const {
    return inputs.find(id) != inputs.end();
}

NODE_API zany INode::get_input(std::string const &id) const {
    // if (has_keyframe(id)) {
...
NODE_API zany INode::resolveInput(std::string const& id) {
...
NODE_API void INode::set_output(std::string const &id, zany obj) {
    outputs[id] = std::move(obj);
}

// NODE_API bool INode::has_keyframe(std::string const &id) const {
//     return kframes.find(id) != kframes.end();
// }
//
// NODE_API zany INode::get_keyframe(std::string const &id) const
// {
...
NODE_API bool INode::has_formula(std::string const &id) const {
    return formulas.find(id) != formulas.end();
}

NODE_API zany INode::get_formula(std::string const &id) const 
{
    return {};
    // auto value = safe_at(inputs, id, "input socket of node `" + myname + "`");
    // if (auto formulas = dynamic_cast<NodeEditor::StringObject *>(value.get()))
    // {
...
// NODE_API TempNodeCaller INode::temp_node(std::string const &id) {
//     return TempNodeCaller(graph, id);
// }

//NODE_API std::variant<int, float, std::string> INode::get_param(std::string const &id) const {
    //auto nid = id + ':';
